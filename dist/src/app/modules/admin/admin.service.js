...
const getPendingCertifications = async () => {
    const vendors = await prisma_1.prisma.vendorProfile.findMany({
        where: { certificationStatus: generated_1.CertificationStatus.Pending },
        include: {
            user: true,
            certifications: true,
        },
...
const approveCertification = async (vendorId) => {
    ...
    const updatedVendor = await prisma_1.prisma.vendorProfile.update({
        where: { id },
        data: { certificationStatus: generated_1.CertificationStatus.Approved },
        include: { user: true },
    });
...
const rejectCertification = async (vendorId, reason) => {
    ...
    const updatedVendor = await prisma_1.prisma.vendorProfile.update({
        where: { id },
        data: { certificationStatus: generated_1.CertificationStatus.Rejected },
        include: { user: true },
    });
