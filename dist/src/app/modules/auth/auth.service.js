    ...
    if (roleString === 'Customer') {
        process.nextTick(async () => {
            try {
                ...
                await NotificationService.createNotification(user.id, 'SYSTEM', 'Welcome to Urban Farming!', `Welcome ${user.name}! Your account has been created successfully. You can now browse products and place orders.`, {
                    userId: user.id,
                    userName: user.name,
