...
const createOrUpdateProfile = async (user, req) => {
    ...
    try {
        ...
        if (!req.body.farmName || !req.body.farmLocation) {
            ...
            throw new ApiError_js_1.default('Farm name and farm location are required', 400);
        }
        ...
        if (req.body.certificationUrls && req.body.certificationUrls.length > 0) {
            ...
            const existingProfile = await prisma_js_1.prisma.vendorProfile.findUnique({
        ...
        const result = await prisma_js_1.prisma.vendorProfile.upsert({
            where: {
                userId: user.id,
            },
            update: profileData,
...
const getProfile = async (user) => {
    const profile = await prisma_js_1.prisma.vendorProfile.findUnique({
        where: {
            userId: user.id,
        },
    });
...
const getVendorCard = async (user) => {
    ...
    return {
        ...
        totalProduces: await prisma_js_1.prisma.produce.count({ where: { vendorId: profile.id } }),
        totalRentalSpaces: await prisma_js_1.prisma.rentalSpace.count({ where: { vendorId: profile.id } }),
...
const createRentalSpace = async (user, req) => {
    ...
    if (!location || !size || !price) {
        throw new ApiError_js_1.default('Location, size, and price are required', 400);
    }
    // Validate price is a valid number
    const parsedPrice = parseFloat(price);
    if (isNaN(parsedPrice) || parsedPrice <= 0) {
        throw new ApiError_js_1.default('Price must be a valid positive number', 400);
    }
    const profile = await prisma_js_1.prisma.vendorProfile.findUnique({
        where: {
            userId: user.id,
    ...
    if (!profile) {
        throw new ApiError_js_1.default('Vendor profile not found', 404);
    }
...
const updateRentalSpace = async (user, id, req) => {
    ...
    const rentalSpace = await prisma_js_1.prisma.rentalSpace.update({
        where: {
            id: parseInt(id),
            vendorId: profile.id,
        },
...
const deleteRentalSpace = async (user, id) => {
    ...
    const rentalSpace = await prisma_js_1.prisma.rentalSpace.delete({
        where: {
            id: parseInt(id),
            vendorId: profile.id,
        },
...
const createProduce = async (user, req) => {
    ...
    if (!validCategories.includes(req.body.category)) {
        throw new ApiError_js_1.default(`Invalid category. Valid categories are: ${validCategories.join(', ')}`, 400);
    }
    const produceData = {
        name: req.body.name,
...
const updateProduce = async (user, id, req) => {
    ...
    const produce = await prisma_js_1.prisma.produce.update({
        where: {
            id: parseInt(id),
            vendorId: profile.id,
        },
...
const deleteProduce = async (user, id) => {
    ...
    const produce = await prisma_js_1.prisma.produce.delete({
        where: {
            id: parseInt(id),
            vendorId: profile.id,
        },
...
const updatePlantStatus = async (user, data) => {
    ...
    if (plantStatus && !validPlantStatuses.includes(plantStatus)) {
        throw new ApiError_js_1.default(400, 'Invalid plant status');
    }
    const updateData = {};
    if (plantStatus)
...
const updateOrderStatus = async (user, orderId, status) => {
    ...
    if (!profile) {
        throw new ApiError_js_1.default(404, 'Vendor profile not found');
    }
    const validStatuses = ['Pending', 'Confirmed', 'Shipped', 'Delivered', 'Cancelled'];
    if (!validStatuses.includes(status)) {
        throw new ApiError_js_1.default(400, 'Invalid order status');
    }
    ...
    if (!order) {
        throw new ApiError_js_1.default(404, 'Order not found');
    }
...
const createVendorPost = async (user, req) => {
    ...
    const post = await prisma_js_1.prisma.vendorPost.create({
        data: postData,
        include: {
            vendor: {
                include: {
...
const getVendorPosts = async (user) => {
    ...
    const posts = await prisma_js_1.prisma.vendorPost.findMany({
        where: {
            vendorId: profile.id,
        },
        include: {
...
const updateVendorPost = async (user, id, req) => {
    ...
    const post = await prisma_js_1.prisma.vendorPost.findUnique({
        where: {
            id: parseInt(id),
        },
    });
    if (!post) {
        throw new ApiError_js_1.default('Vendor post not found', 404);
    }
    // Check if the post belongs to the user
    if (post.vendorId !== profile.id) {
        throw new ApiError_js_1.default('You can only update your own posts', 403);
    }
    // Validate category if provided
    ...
    const updatedPost = await prisma_js_1.prisma.vendorPost.update({
        where: {
            id: parseInt(id),
        },
        data: updateData,
...
const deleteVendorPost = async (user, id) => {
    ...
    // Check if the post belongs to the user
    if (post.vendorId !== profile.id) {
        throw new ApiError_js_1.default('You can only delete your own posts', 403);
    }
    await prisma_js_1.prisma.vendorPost.delete({
        where: {
            id: parseInt(id),
        },
    });
...
const toggleVendorPostLike = async (user, postId) => {
    ...
    const existingLike = await prisma_js_1.prisma.vendorPostLike.findUnique({
        where: {
            userId_postId: {
                userId: user.id,
    ...
    if (existingLike) {
        // Unlike the post
        await prisma_js_1.prisma.vendorPostLike.delete({
        ...
        // Like the post
        await prisma_js_1.prisma.vendorPostLike.create({
            data: {
                userId: user.id,
                postId: parseInt(postId),
            },
...
const addVendorPostComment = async (user, postId, content) => {
    ...
    if (!content.trim()) {
        throw new ApiError_js_1.default('Comment content cannot be empty', 400);
    }
    const comment = await prisma_js_1.prisma.vendorPostComment.create({
        data: {
            userId: user.id,
            postId: parseInt(postId),
...
const getVendorDashboardStats = async (user) => {
    ...
    const totalSalesResult = await prisma_js_1.prisma.order.aggregate({
        where: {
            status: 'Delivered',
            vendorId: profile.id,
        },
    ...
    const activeRentals = await prisma_js_1.prisma.rentalSpace.count({
        where: {
            vendorId: profile.id,
            availability: true,
        },
    ...
    // Get pending orders
    const pendingOrders = await prisma_js_1.prisma.order.count({
        where: {
            status: 'Pending',
            vendorId: profile.id,
        },
    ...
    // Get total products
    const totalProducts = await prisma_js_1.prisma.produce.count({
        where: {
            vendorId: profile.id,
        },
    });
    ...
    for (let i = 3; i >= 0; i--) {
        ...
        const monthlySales = await prisma_js_1.prisma.order.aggregate({
            where: {
                status: 'Delivered',
                vendorId: profile.id,
                orderDate: {
