  ...
  process.nextTick(async () => {
    try {
      ...
      await NotificationService.createNotification(
        ...
        'Rental Order Status Updated',
        `Your rental order #${updated.id} status has been updated to ${status}.`,
        {
          orderId: updated.id,
          status: status,
      ...
      // Notify vendor
      await NotificationService.createNotification(
        updated.rentalSpace.vendor.userId,
        'ORDER_STATUS_UPDATE' as any,
        'Rental Order Status Updated',
        `Rental order #${updated.id} status has been updated to ${status}.`,
        {
          orderId: updated.id,
          status: status,
    ...
    try {
      ...
      if (customer) {
        await NotificationService.createNotification(
          customer.id,
          'RENTAL_SPACE_BOOKED' as any,
          'Rental Space Booked Successfully',
          `You have successfully booked the rental space "${updated.location}".`,
          {
            rentalSpaceId: updated.id,
...
const createRentalOrder = async (customerId: string, spaceId: number, totalPrice: number, duration?: number) => {
...
const getVendorRentalOrders = async (vendorId: string) => {
  const vendorIdNumber = parseInt(vendorId, 10);
  if (isNaN(vendorIdNumber)) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid vendor ID');
  }
...
const updateRentalOrderStatus = async (orderId: number, status: string, vendorId: string) => {
  const vendorIdNumber = parseInt(vendorId, 10);
  if (isNaN(vendorIdNumber)) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid vendor ID');
  }
